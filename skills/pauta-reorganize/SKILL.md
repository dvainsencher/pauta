...
description: This skill should be used when the user wants to restructure a pauta-managed backlog — phrases like "reorganize the backlog", "regroup these into sprints", "clean up the roadmap", "rebatch these items", or "this sprint is getting too big, split it".
---
...
1. Run `pauta show --json` to read the whole plan: every backlog item, every sprint
...
2. Reason about a regrouping that serves the user's stated goal (e.g. "these five
   backlog items are really one feature," "this sprint has twelve items, split it
...
4. Once the user confirms (or if they already gave you clear enough instructions
   ...
   - `pauta move <id> <sprint-name>` / `pauta move <id> --backlog` to reassign items
...
5. Do not delete or rename sprints — there's no `remove-sprint` command; if a sprint
   is no longer useful, move its items elsewhere and leave it `done`/empty rather
