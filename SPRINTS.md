...
#113  idea   `docs/roadmap/` namespace-collision fix — confirmed real during the dry run: a project with its own pre-existing `docs/roadmap/<slug>.md` files ends up with those files mixed alongside pauta's `issues.jsonl`/`sprints.json`/`specs/` after `init`. Better than the port-then-quarantine sequencing originally tried (which leaves files mixed in for the whole migration, and risks quarantining content that hasn't actually been ported yet): rename the *whole* legacy directory out of the way *before* running `pauta init` (e.g. `git mv docs/roadmap docs/roadmap-legacy`), so pauta's `docs/roadmap/` is clean from minute one and legacy content is ported from `docs/roadmap-legacy/` at leisure, with the rename itself as the only required step (no per-file quarantine bookkeeping, no risk of mixed ownership at any point). Document this rename-first lifecycle in the future #111 migration case; only add a configurable data path (env var/`.pautarc`) if that proves insufficient.
