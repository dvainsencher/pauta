...
#129  done   Generated `ROADMAP.md` — every mutating command (`init`, `add-issue`, `edit-issue`, `import`, `remove-issue`, `create-sprint`, `edit-sprint`, `remove-sprint`, `move`, `set-status`, `set-sprint-status`, `set-active`, `set-position`) now also regenerates a human-readable `ROADMAP.md` at the project root (backlog + one checklist section per sprint, done issues checked off), so a project tracked with scrummy gets a browsable doc on GitHub without anyone running the CLI. New `scrummy roadmap` command regenerates it on demand. Marked `_Generated by scrummy — do not edit by hand_`; `scrummy-bootstrap`'s legacy-doc detection (#111) now recognizes and skips that marker instead of mistaking it for a hand-rolled backlog to migrate.
```
